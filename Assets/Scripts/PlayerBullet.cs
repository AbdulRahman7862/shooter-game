...
public class PlayerBullet : MonoBehaviour
{
    float bulletSpeed = 8f; // Speed of the bullet
    public AudioClip hitSound; // Assign an audio clip for the hit sound
    private AudioSource audioSource;
    ...
    void Update()
    {
        ...
        Vector2 pos = transform.position;
        pos.y -= bulletSpeed * Time.deltaTime; // Change to move downward
        transform.position = pos;

        // Destroy bullet if it goes out of the screen
        Vector2 minPosition = Camera.main.ViewportToWorldPoint(new Vector2(0, 0)); // Bottom-left of the screen
        if (transform.position.y < minPosition.y) // Check if below the screen
        {
            Destroy(gameObject);
        }
    }
